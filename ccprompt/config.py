...
class Config:
    ...
    def load_config(self):
        ...
        if not os.path.exists(self.config_file):
            ...
            print(f"No configuration file found. Creating default config file at {self.config_file}.")
        else:
            if os.path.getsize(self.config_file) == 0:
                config_needs_creation = True
                print(f"Configuration file {self.config_file} is empty. Writing default configurations.")
        ...
        if config_needs_creation:
            try:
                ...
                print(f"Error creating default configuration file {self.config_file}: {e}")
                sys.exit(1)
        ...
        self.target_name = self.args.target_names if self.args.target_names else config.get('target_name', [])
        self.project_path = self.args.project_path if self.args.project_path else config.get('project_path', "")
        ...
        self.output_file = self.args.output_file if self.args.output_file else config.get('output_file', 'extracted_code.txt')
        self.language = self.args.language if self.args.language else config.get('language', 'python')
        ...
        if not self.target_name or not self.project_path:
            ...
            print(f"Please update the configuration file at {self.config_file} or provide the required parameters via command-line.\n")
            print("Example command-line usage:")
            print("  ccprompt --target_names MyFunction MyClass --project_path /path/to/project\n")
            sys.exit(1)